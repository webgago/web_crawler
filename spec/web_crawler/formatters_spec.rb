...
describe WebCrawler::Formatter::Csv do
  ...
  it "should format input array to csv string" do
    described_class.new(input).draw.should == "1,2,3\nstring,\"other string\n\"\n"
  end

  it "should format input hash to csv string" do
    described_class.new(input_hash).draw.should == "title,url,author\n1,2,3\nstring,\"other string\n\"\n"
  end

  it "should format input array to csv string with options" do
    described_class.new(input, headers: [:title, :url, :author], col_sep: ";").draw.should == "title;url;author\n1;2;3\nstring;\"other string\n\"\n"
    described_class.new(input, headers: [:title, :url, :author], row_sep: "\n\n").draw.should == "title,url,author\n\n1,2,3\n\nstring,\"other string\n\"\n\n"
  end
...
describe WebCrawler::Formatter::Json do
  ...
  it "should format input array to json string" do
    described_class.new(input, headers: [:title, :url, :author]).draw.should == '{"responses":[[1,2,"3"],["string","other string\n"]]}'
  end

  it "should format input hash to json string" do
    json = described_class.new(input_hash).draw
...
describe WebCrawler::Formatter::Xml do
  ...
  it "should format input array to xml string" do
    xml = "<responses>" <<
        ...
        "</responses>"
    described_class.new(input, headers: [:title, :url, :author]).draw.should == xml
  end
  ...
  it "should format input array to pretty xml string" do
    xml = "<responses>\n" <<
        ...
        "<response><title>string</title><url>other string\n</url><author></author></response>\n" <<
        "</responses>"
    described_class.new(input, headers: [:title, :url, :author], pretty: true).draw.should == xml
  end
  ...
  it "should format input array without :headers to xml string" do
    xml = "<responses>\n" <<
        ...
        "</responses>"
    described_class.new(input, pretty: true).draw.should == xml
  end
  ...
  it "should format input hash to xml string" do
    xml = "<responses>\n" <<
        ...
        "<response><title>string</title><url>other string\n</url><author></author></response>\n" <<
        "</responses>"
    described_class.new(input_hash, pretty: true).draw.should == xml
  end
end